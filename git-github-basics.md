Option 1: Using VS Code (simplest)

Open essay.txt in VS Code (it should already be open if you're in the same folder).
Paste the content below into the file.
Save it (Ctrl+S).

Option 2: Using terminal directly
Run this in PowerShell/cmd from inside your practicalgit folder:

notepad essay.txt

This opens the file in Notepad — paste the content, save, and close.

Here's the content to paste into essay.txt:
...
4. In VS Code's terminal (not the code editor area), run:
...
3. You only want to save notes.txt right now (maybe todo.txt isn't ready). Run:
   git add notes.txt
